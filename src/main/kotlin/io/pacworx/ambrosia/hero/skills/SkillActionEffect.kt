...
enum class SkillActionEffect(val type: SkillActionType,
    ...
    STRENGTH_SCALING(ADD_BASE_DMG, "Adds base damage scaling with strength"),
    ARMOR_SCALING(ADD_BASE_DMG, "Adds base damage scaling with current armor"),
    ARMOR_MAX_SCALING(ADD_BASE_DMG, "Adds base damage scaling with max armor"),
    HP_SCALING(ADD_BASE_DMG, "Adds base damage scaling with current hitpoints"),
    HP_MAX_SCALING(ADD_BASE_DMG, "Adds base damage scaling with max hitpoints"),
    DEXTERITY_SCALING(ADD_BASE_DMG, "Adds base damage scaling with dexterity"),
    RESISTANCE_SCALING(ADD_BASE_DMG, "Adds base damage scaling with resistance"),
    HERO_LVL_SCALING(ADD_BASE_DMG, "Adds base damage scaling with hero level"),
    DMG_MULTIPLIER(ADD_BASE_DMG, "Adds percentage base damage. E.g. 10 for +10% damage"),
    FIXED_DMG(ADD_BASE_DMG, "Adds fixed base damage"),
    ...
    CRIT_BUFF(BUFF, "Grants Crit Buff with value intensity for duration turn", Buff.CRIT_BUFF),
    CRIT_MULT_BUFF(BUFF, "Grants CritMult Buff with value intensity for duration turn", Buff.CRIT_MULT_BUFF),
    DEATHSHIELD_BUFF(BUFF, "Grants Deathshield Buff with value intensity for duration turn", Buff.DEATHSHIELD),
    DODGE_BUFF(BUFF, "Grants Dodge Buff with value intensity for duration turn", Buff.DODGE_BUFF),
    HASTE_BUFF(BUFF, "Grants Haste Buff with value intensity for duration turn", Buff.HASTE),
    ...
    CRIT_DEBUFF(DEBUFF, "Applies Crit Debuff with value intensity for duration turn", Buff.CRIT_DEBUFF),
    CRIT_MULT_DEBUFF(DEBUFF, "Applies CritMult Debuff with value intensity for duration turn", Buff.CRIT_MULT_DEBUFF),
    ...
    SLOW_DEBUFF(DEBUFF, "Applies Slow Debuff with value intensity for duration turn", Buff.SLOW),
